...
DataBatch: TypeAlias = tuple[npt.NDArray[np.float32], npt.NDArray[np.float32], npt.NDArray[np.float32]]
...
def load_jsonl(filepath: str | Path) -> DataBatch:
    """
    ...
    """
    # Typed Any to avoid a reassignment conflict: these start as lists and are
    # later replaced by np.ndarray via np.array(...).
    states: Any = []
    policies: Any = []
    values: Any = []
...
def load_multiple_jsonl(filepaths: list[str | Path]) -> DataBatch:
    """
...
def compute_entropy(policies: npt.NDArray[np.float32]) -> float:
    """
...
def load_preprocessed(filepath: str | Path) -> DataBatch:
    """
